...
O demonstrador é quem executa a ação que irá ser aprendida. A escolha deste também pode ser quebrada em outras duas categorias: quem controla a ação e quem executa a demonstração. Geralmente os demonstradores são humanos, mas também existem estudos que utilizam outros robôs ou simuladores no papel de instrutores. 

As técnicas para realizar uma demonstração podem ser dividas em quatro grandes categorias:
\begin{itemize}
\item Tele-operação: o instrutor opera o robô e este grava a execução. O professor pode tanto controlar o robô remotamente, quanto aplicar o \textit{kinesthetic teaching} (o robô passivamente é controlado por um humano que mexe suas juntas - vide figura \ref{fig:kinesthetic}).
